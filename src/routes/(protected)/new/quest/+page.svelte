...
<div class="xl:h-full lg:h-full grid grid-cols-1 p-8 lg:grid-cols-2 xl:grid-cols-2 book">
    <div class="book-left flex flex-col">
        <h1 class="font-thick text-6xl text-amber-900 mt-[-2rem]">Quest</h1>
                ...
                <div class="flex gap-3 items-center">
                    {#if questTimingOption.questTiming.label === 'date'}
                        <p class="text-2xl italic text-gray-700">
                            {questTimingOption.date || questTimingOption.date.toLocaleDateString()}
                        </p>
                    {:else if questTimingOption.questTiming.label === 'time'}
                        <p class="text-2xl italic text-gray-700">
                            {questTimingOption.time.days}d {questTimingOption.time.hours}h {questTimingOption.time
                            .minutes}m
                        </p>
                    {:else}
        ...
        <div class="flex-1 flex flex-col">
            ...
            <textarea
                    ...
                    maxlength="500"
                    class="h-full mb-3.5 flex-1 resize-none w-full desc p-3 noOutline min-h-[300px]"
            />
        </div>
    </div>
    <div class="book-right flex grid grid-rows-[auto_500px_auto] h-full">
        <div class="flex items-center mt-[-2rem] mb-2">
        ...
        <div class="overflow-x-hidden">
            <div class="overflow-y-auto h-full py-4 flex gap-2 flex-col"
                 style="--scroll-bar-color-primary: #ffefd6; --scroll-bar-color-secondary: #ffd89f; --scrollbar-width: 10px;">
                {#each taskList as task, i}
                    <div class="h-[content] flex shadow rounded card w-full shrink-0">
                        ...
                        >
                            <img class="h-8" src="/quest/boolean/false.png" alt="X"/>
                        </button>
                    </div>
                {/each}
            </div>
        ...
        <button
                ...
                class="font-thick cool-button mb-4"
        >Submit
        </button>
    </div>
</div>
    ...
    <div class="flex">
        <div class="flex-1">
            {#if questTimingOption.questTiming.label === 'date'}
                <input
                        type="date"
                        class="p-2 m-3 text-3xl noOutline border-none rounded shadow flex-1"
